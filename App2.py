...
QUEUE_BOARD_URL = os.environ.get("QUEUE_BOARD_URL", "http://localhost:8502")


def make_queue_board_qr_svg(text):
    qr = qrcode.QRCode(
        version=None,
...
with st.container():
    ...
    if st.session_state["submitted_result"] is None:
        ...
        st.markdown(
            """
            <div class="qr-panel">
              <h4>Open the Queue Board</h4>
              <p class="helper-text" style="margin-top:0;">
                Scan this QR code or open the direct link to go to the staff queue board.
              </p>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        with qr_col:
            st.markdown(
                f'<div class="qr-svg">{make_queue_board_qr_svg(QUEUE_BOARD_URL)}</div>',
                unsafe_allow_html=True,
            )
            st.caption("Queue board QR code")
        with link_col:
            st.markdown(
                f"""
                <div class="qr-panel" style="margin-top:0;">
                  <h4>Queue Board URL</h4>
                  <code>{html.escape(QUEUE_BOARD_URL)}</code>
                </div>
                """,
                unsafe_allow_html=True,
            )
            st.link_button("Open Queue Board", QUEUE_BOARD_URL, use_container_width=True)
