...
def remove_pending(patient_id):
    pending = [p for p in load_pending() if p["id"] != patient_id]
...
CTAS_WAIT = {1: 0, 2: 15, 3: 30, 4: 60, 5: 120}
...
st.markdown(
    ...
    .block-container { max-width: 980px !important; padding-top: 1.5rem; padding-bottom: 2.5rem; }
    .hero-header {
...
with tab_queue:
    ...
    def render_er_column(col, er):
        with col:
            ...
            if not patients:
                st.markdown('<div class="empty-state">🟢<br>No patients in queue</div>', unsafe_allow_html=True)
                return
...
with tab_add:
    ...
    if submitted:
        if not name.strip():
            ...
            st.session_state.add_success = f"✅ {name.strip()} added to {er} queue — Ticket #{st.session_state.counters[er]}"
            st.rerun()
